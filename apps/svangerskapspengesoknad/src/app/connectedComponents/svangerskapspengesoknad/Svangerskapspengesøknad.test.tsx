...
describe('<Svangerskapspengesøknad>', () => {
  it('skal rendre komponent ok', async () => {
    ...
    expect(await screen.findByText('For å gå videre med søknaden må du bekrefte at du har lest og forstått')).toBeInTheDocument();

    await userEvent.click(screen.getByText('Begynn med søknad'));

    expect(await screen.findByText('Du må godkjenne vilkårene for å kunne fortsette med søknaden.')).toBeInTheDocument();
