...
describe('<ArbeidsforholdOgInntektSteg>', () => {
    ...
    it(
        'skal gi valideringsmelding når inputs ikke er utfylt',
        mswWrapper(async ({ setHandlers }) => {
            const gåTilNesteSide = vi.fn();
            const mellomlagreSøknadOgNaviger = vi.fn();
            const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });
            ...
            expect(screen.getAllByText('Du må oppgi om du har arbeidet som frilanser de siste 4 ukene.')).toHaveLength(
                2,
            );
            expect(
                screen.getAllByText(
    ...
    it(
        'skal ikke kunne søke når det er ingen aktive arbeidsforhold og en svarer nei på frilans og selvstendig næringsdrivende',
        mswWrapper(async ({ setHandlers }) => {
            const gåTilNesteSide = vi.fn();
            const mellomlagreSøknadOgNaviger = vi.fn();
            const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });
