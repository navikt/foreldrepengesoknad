...
import { ContextDataMap, ContextDataType, FpDataContext, useContextComplete, useContextReset } from './FpDataContext';
...
it('mellomlagrer ferske skjemaverdier og vedlegg uten å gjenbruke dem i en ny søknad', async () => {
    ...
    const { result, unmount } = renderHook(
        () => ({
            ...
            lagre: useMellomlagreSøknad([], søkerInfo, false).mellomlagreSøknad,
            state: useContextComplete(),
        }),
        { wrapper: lagWrapper({ [ContextDataType.APP_ROUTE]: SøknadRoutes.OM_BARNET }) },
    );
