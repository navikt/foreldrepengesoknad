...
const LINK_TIL_FARSKAPSPORTAL_MOR =
    'Hvis du og faren til barnet er gift, blir han automatisk registrert som far til barnet';
// const LINK_TIL_FARSKAPSPORTAL_FAR = 'Hvis du og moren til barnet er gift, blir du automatisk registrert som far til barnet';
describe('<AnnenForelder>', () => {
    ...
    //     expect(screen.getByText(GÅ_VIDERE_KNAPP)).toBeInTheDocument();
    // });
    it('skal vise infoboks om farskapsportal når mor søker på termin, annen forelder er far og har rett i Norge eller når far har utenlandsk fnr', async () => {
        const user = userEvent.setup();
        render(<MorUfødtBarn />);
        expect(await screen.findByText(NAVN_ANNEN_FORELDER_LABEL)).toBeInTheDocument();
        expect(screen.queryByText(GÅ_VIDERE_KNAPP)).not.toBeInTheDocument();
    ...
    // it('skal vise infoboks om farskapsportal når far søker på termin og kun når mor har ikke rett i Norge', async () => {
    //     const user = userEvent.setup();
    //     render(<FarUfødtBarn />);
    //     expect(await screen.findByText(NAVN_ANNEN_FORELDER_LABEL)).toBeInTheDocument();
    //     const fornavnInput = screen.getByLabelText('Fornavn:');
    ...
    //     expect(await screen.findByText(GÅ_VIDERE_KNAPP)).toBeInTheDocument();
    // });
    // it('skal vise infoboks om farskapsportal når medmor søker', async () => {
    //     const user = userEvent.setup();
