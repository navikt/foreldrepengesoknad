...
const BarnInnlagtUploader: FunctionComponent<Props> = ({
    attachments,
    updateAttachments,
    perioder,
    navnPåForeldre,
    ...
    const formAttachments = watch(Skjemanummer.DOK_INNLEGGELSE_BARN);

    useEffect(() => {
        if (formAttachments.length === 0) {
            const init = lagSendSenereDokument(AttachmentType.UTSETTELSE_SYKDOM, Skjemanummer.DOK_INNLEGGELSE_BARN);
    ...
    return (
        <VStack gap="4">
            <div>
                <Label>Dokumentasjon på at barnet er innlagt</Label>
                <BodyLong>Du må laste opp dokumentasjon på at barnet er innlagt på sykehus</BodyLong>
                {perioder.map((p) => {
                    return (
...
export default BarnInnlagtUploader;
