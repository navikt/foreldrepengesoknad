...
const OverføringsUploader: FunctionComponent<Props> = ({
    ...
    const formAttachments = watch(Skjemanummer.DOK_OVERFØRING_FOR_SYK);

    useEffect(() => {
        if (formAttachments.length === 0) {
            const init = lagSendSenereDokument(AttachmentType.OVERFØRING_KVOTE, Skjemanummer.DOK_OVERFØRING_FOR_SYK);
