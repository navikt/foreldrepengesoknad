...
describe('<PlanenDeresSteg - fødsel>', () => {
    it('skal vise korrekt data for fødsel - mor og far - begge har rett', async () => {
        ...
        expect(within(oktober).getByTestId('day:14;dayColor:GREEN;dayType:FIRST_DAY')).toBeInTheDocument();
        ...
        expect(within(mai2025).getByTestId('day:16;dayColor:GREEN;dayType:LAST_DAY')).toBeInTheDocument();
    ...
    it('skal vise korrekt data for fødsel - mor og far - kun mor har rett', async () => {
        ...
        const juli = screen.getByTestId('year:2024;month:6');
        expect(within(juli).getByTestId('day:1;dayColor:PINK;dayType:FIRST_AND_LAST_DAY')).toBeInTheDocument();
        expect(within(juli).getByTestId('day:2;dayColor:LIGHTBLUE;dayType:FIRST_DAY')).toBeInTheDocument();
        expect(within(juli).getAllByTestId('dayColor:LIGHTBLUE', { exact: false })).toHaveLength(22);

        const mai2025 = screen.getByTestId('year:2025;month:4');
        expect(within(mai2025).getByTestId('day:16;dayColor:LIGHTBLUE;dayType:LAST_DAY')).toBeInTheDocument();
        expect(within(mai2025).getAllByTestId('dayColor:LIGHTBLUE', { exact: false })).toHaveLength(12);
    });
    ...
    it('skal vise korrekt data for fødsel - far søker og har rett', async () => {
        ...
        const juli = screen.getByTestId('year:2024;month:6');
        expect(within(juli).getByTestId('day:1;dayColor:PINK;dayType:FIRST_AND_LAST_DAY')).toBeInTheDocument();
        expect(within(juli).getByTestId('day:2;dayColor:LIGHTGREEN;dayType:FIRST_DAY')).toBeInTheDocument();
        expect(within(juli).getAllByTestId('dayColor:LIGHTGREEN', { exact: false })).toHaveLength(22);
    ...
    it('skal vise korrekt data for fødsel - far og far søker - kun medfar har rett', async () => {
        ...
        expect(within(mai2005).getAllByTestId('dayColor:LIGHTGREEN', { exact: false })).toHaveLength(12);
    });
    ...
    it('skal endre dekningsgrad til 80%', async () => {
        ...
        expect(within(nov).getByTestId('day:11;dayColor:GREEN;dayType:FIRST_DAY')).toBeInTheDocument();
    ...
    it('skal endre fordeling av fellesperiode sånn at de får 8 uker hver', async () => {
        ...
        expect(within(desember).getByTestId('day:9;dayColor:GREEN;dayType:FIRST_DAY')).toBeInTheDocument();
