...
describe('<Planlegger>', () => {
    mswTest('skal gå rett til oppsummering når ingen av foreldrene har rett', async ({ setHandlers }) => {
    ...
    mswTest('skal gå rett til oppsummering når barnet er født for mer enn tre år siden', async ({ setHandlers }) => {
        setHandlers(DefaultMockaStønadskontoerOgSatser.parameters.msw);
        const utils = render(<DefaultMockaStønadskontoerOgSatser />);
        ...
        await userEvent.type(termindato, dayjs().subtract(20, 'day').format(DDMMYYYY_DATE_FORMAT));
        fireEvent.blur(termindato);
        // TODO (TOR) Dette feilar av og til: screen.findAllByText('Oppsummering')
        // await userEvent.click(screen.getByText('Neste'));
    ...
    mswTest('skal ikke vise fordelingssteget når far og far og barnet er født', async ({ setHandlers }) => {
    ...
    mswTest('skal ikke vise barnehageplass-steget når adopsjon', async ({ setHandlers }) => {
        setHandlers(DefaultMockaStønadskontoerOgSatser.parameters.msw);
        const utils = render(<DefaultMockaStønadskontoerOgSatser />);
