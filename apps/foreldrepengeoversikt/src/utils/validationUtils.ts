...
const validateTextInputField = (value: string, feltNavn: string, intl: IntlShape): SkjemaelementFeil => {
    if (!textRegex.test(value)) {
        ...
        return intl.formatMessage({ id: 'validationUtils.fritekst.kanIkkeInneholdeTegn' }, { feltNavn, ugyldigeTegn });
    }
    return null;
};
...
export const validateFritekstFelt = (intl: IntlShape, label: string, inputText?: string) => {
    if (inputText === undefined || inputText === null || inputText.length === 0 || inputText.length < 25) {
        return intl.formatMessage({ id: 'validationUtils.fritekst.forKort' }, { feltNavn: label });
    }

    if (inputText.length > 1000) {
        return intl.formatMessage({ id: 'validationUtils.fritekst.forLang' }, { feltNavn: label });
    }
