...
describe('UttaksplanKalender', () => {
    ...
    it('skal markere en periode som ikke overlapper med eksisterende perioder', async () => {
        ...
        expect(
            screen.getByText(
                "Du har markert dager som ikke er lagt til i planen ennå. Velg 'Ny periode' for å opprette en ny periode.",
            ),
        ).toBeInTheDocument();
    ...
    it('skal vise advarsel om at en mister dager om en velger å gradere i seksukersperioden etter fødsel som mor', async () => {
        ...
        await userEvent.click(screen.getAllByText('Hva vil du endre til?')[3]!);
        await userEvent.click(screen.getByText('Ny periode'));
    ...
    it('skal vise pause og legge til ny pause', async () => {
        ...
        await userEvent.click(screen.getByText('Legg til pause'));

        expect(screen.getByText('Aktivitetskrav gjelder i denne perioden')).toBeInTheDocument();

        const aktivitetskravSelect = screen.getByLabelText('Hva gjør mor i denne perioden?');
        await userEvent.selectOptions(aktivitetskravSelect, 'ARBEID');
