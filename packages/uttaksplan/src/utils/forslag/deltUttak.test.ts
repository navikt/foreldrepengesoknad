...
describe('deltUttak - Fødsel', () => {
    ...
    it('skal gi far/medmor fellesperioden som ikke blir brukt av mor', () => {
        ...
        expect(forslag[1]!.søker!.kontoType).toEqual('MØDREKVOTE');
        expect(forslag[2]!.fom).toEqual('2022-09-12');
        expect(forslag[2]!.tom).toEqual('2022-11-04');
        expect(forslag[2]!.søker!.forelder).toEqual('MOR');
        expect(forslag[2]!.søker!.kontoType).toEqual('FELLESPERIODE');

        expect(forslag[3]!.fom).toEqual('2022-11-07');
        expect(forslag[3]!.tom).toEqual('2022-12-09');
        expect(forslag[3]!.søker!.forelder).toEqual('FAR_MEDMOR');
        expect(forslag[3]!.søker!.kontoType).toEqual('FEDREKVOTE');
        expect(forslag[4]!.fom).toEqual('2022-12-12');
        expect(forslag[4]!.tom).toEqual('2023-02-03');
        expect(forslag[4]!.søker!.forelder).toEqual('FAR_MEDMOR');
        expect(forslag[4]!.søker!.kontoType).toEqual('FELLESPERIODE');
    });
    ...
    it('skal gi far all fellesperiode og ingen fellesperiode til mor', () => {
        ...
        expect(forslag[1]!.søker!.kontoType).toEqual('MØDREKVOTE');
        expect(forslag[1]!.søker!.forelder).toEqual('MOR');
        expect(forslag[2]!.søker!.kontoType).toEqual('FEDREKVOTE');
        expect(forslag[2]!.søker!.forelder).toEqual('FAR_MEDMOR');
        expect(forslag[2]!.fom).toEqual('2022-09-12');
        expect(forslag[2]!.tom).toEqual('2022-10-14');
        expect(forslag[3]!.søker!.kontoType).toEqual('FELLESPERIODE');
        expect(forslag[3]!.søker!.forelder).toEqual('FAR_MEDMOR');
    ...
    it('skal legge inn fellesperiode før FPFF når startdato er mer enn 3 uker før fødsel', () => {
        ...
        expect(forslag[0]!.søker!.kontoType).toEqual('FELLESPERIODE');
        expect(forslag[0]!.søker!.forelder).toEqual('MOR');
        expect(forslag[0]!.fom).toEqual('2022-07-11');
        expect(forslag[0]!.tom).toEqual('2022-07-15');
        expect(forslag[1]!.søker!.kontoType).toEqual('FORELDREPENGER_FØR_FØDSEL');
        expect(forslag[1]!.søker!.forelder).toEqual('MOR');
        expect(forslag[1]!.fom).toEqual('2022-07-18');
        expect(forslag[1]!.tom).toEqual('2022-08-05');
        expect(forslag[2]!.søker!.kontoType).toEqual('MØDREKVOTE');
        expect(forslag[2]!.søker!.forelder).toEqual('MOR');
        expect(forslag[2]!.fom).toEqual('2022-08-08');
        expect(forslag[3]!.søker!.kontoType).toEqual('FELLESPERIODE');
        expect(forslag[3]!.søker!.forelder).toEqual('MOR');
        expect(forslag[3]!.fom).toEqual('2022-09-12');
        expect(forslag[3]!.tom).toEqual('2022-10-28');
        expect(forslag[4]!.søker!.kontoType).toEqual('FEDREKVOTE');
        expect(forslag[4]!.søker!.forelder).toEqual('FAR_MEDMOR');
        expect(forslag[5]!.søker!.kontoType).toEqual('FELLESPERIODE');
        expect(forslag[5]!.søker!.forelder).toEqual('FAR_MEDMOR');
...
describe('deltUttak - Adopsjon', () => {
    ...
    it('skal starte planen på omsorgsovertakelsesdatoen uten FPFF-periode', () => {
        ...
        expect(forslag[0]!.søker!.kontoType).toEqual('MØDREKVOTE');
        expect(forslag[0]!.søker!.forelder).toEqual('MOR');
        expect(forslag[0]!.fom).toEqual('2022-08-08');
        expect(forslag[0]!.tom).toEqual('2022-09-09');
        expect(forslag[1]!.søker!.kontoType).toEqual('FELLESPERIODE');
        expect(forslag[1]!.søker!.forelder).toEqual('MOR');
    ...
    it('skal legge fars kvote først når far eller medmor er valgt som starter ved adopsjon', () => {
        ...
        expect(forslag.length).toEqual(4);
        expect(forslag[0]!.søker!.kontoType).toEqual('FEDREKVOTE');
        expect(forslag[0]!.søker!.forelder).toEqual('FAR_MEDMOR');
        expect(forslag[0]!.fom).toEqual('2022-08-08');
        expect(forslag[0]!.tom).toEqual('2022-09-09');
        expect(forslag[1]!.søker!.kontoType).toEqual('FELLESPERIODE');
        expect(forslag[1]!.søker!.forelder).toEqual('FAR_MEDMOR');
        expect(forslag[1]!.fom).toEqual('2022-09-12');
        expect(forslag[1]!.tom).toEqual('2022-11-04');
        expect(forslag[2]!.søker!.kontoType).toEqual('MØDREKVOTE');
        expect(forslag[2]!.søker!.forelder).toEqual('MOR');
