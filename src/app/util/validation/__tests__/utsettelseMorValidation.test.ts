...
import { harMorHarSøktUgyldigUtsettelseFørsteSeksUker } from '../uttaksplan/utsettelseMorValidation';
...
describe('Validering av mors uttak første 6 uker', () => {
    it('skal godta utsettelse på grunn av sykdom', () => {
        const result = harMorHarSøktUgyldigUtsettelseFørsteSeksUker(
    ...
    it('Skal godta utsettelse etter 6 uker', () => {
        ...
        const result = harMorHarSøktUgyldigUtsettelseFørsteSeksUker([utsettelseEtter6uker], familiehendelsesdato);
        expect(result).toBeFalsy();
    });
});
